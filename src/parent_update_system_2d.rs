...
pub fn build() -> impl ParallelRunnable {
    SystemBuilder::<()>::new("ParentUpdateSystem2D")
        ...
        .with_query(<(Entity, Read<Children>)>::query().filter(!component::<LocalToWorld2>()))
            ...
            for (entity, children) in queries.2.iter(world) {
                ...
                if children_additions.remove(&entity).is_none() {
                    ...
                    for child_entity in children.0.iter() {
                        ...
                        commands.remove_component::<LocalToParent2>(*child_entity);
                    }
...
mod test {
    use super::*;
    use crate::missing_previous_parent_system_2d;
